...
Aufgabe2a::Aufgabe2a() : Screen(SceneName::Aufgabe2a) {
	...
	invInterface = std::make_unique<InventoryScreen>(sceneName);
}

void Aufgabe2a::Input()
{
	if (!inventoryIsOpen) {
		if (IsKeyReleased(KEY_BACKSPACE)) nextScreen = std::make_shared<StartScreen>();
		if (IsKeyReleased(KEY_E)) inventoryIsOpen = true;
	}
	else {
